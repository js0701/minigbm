...
static int gralloc0_unlock(struct gralloc_module_t const *module, buffer_handle_t handle)
{
	...
	if (fence_fd >= 0) {
		...
		if (ret < 0) {
			cros_gralloc_error("Sync wait timed out, err = %s", strerror(errno));
			return CROS_GRALLOC_ERROR_UNSUPPORTED;
		}
...
static int cros_gralloc_lock_async(struct gralloc_module_t const *module, buffer_handle_t handle,
...
static int cros_gralloc_unlock_async(struct gralloc_module_t const *module, buffer_handle_t handle,
				     int *fence_fd)
{
	auto mod = (struct gralloc0_module *)module;
...
static int cros_gralloc_lock_async_ycbcr(struct gralloc_module_t const *module,
					 buffer_handle_t handle, int usage, int l, int t, int w,
					 int h, struct android_ycbcr *ycbcr, int fence_fd)
{
	uint64_t flags;
	int32_t ret;
...
struct gralloc0_module HAL_MODULE_INFO_SYM = {
	.base =
		...
		.lockAsync = cros_gralloc_lock_async,
		.unlockAsync = cros_gralloc_unlock_async,
		.lockAsync_ycbcr = cros_gralloc_lock_async_ycbcr,
	    },
